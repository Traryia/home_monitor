...
2026-08-15 (晚)  摄像头夜间断电节电 (04:30~18:00 才运行)

  [需求] 用户: 摄像头只需白天工作, 夜间关闭节省电量

  [实现] (192.168.3.41)
    * pi_cam/camera_stream_day.sh: ustreamer 包装脚本, 非 04:30~
      18:00 直接 exit 0 (正常退出, 不触发 Restart 重试循环;
      若用 ExecStartPre 失败退出会被 Restart=on-failure 每3s重试)
    * camera-stream.service ExecStart 改为包装脚本
    * camera-day-on.timer: 04:30 启动 camera-stream.service
    * camera-day-off.{service,timer}: 18:00 systemctl stop
    * 夜间开机/插拔 USB 均被闸门拦截, 不会意外开摄像头

  [验证] 部署时 19:12 (窗口外), 服务 restart 后正确保持 inactive;
    ustreamer 进程已停; 定时器下次触发 04:30/18:00 正确

  [注意] 夜间点桌面「摄像头」图标: 服务会被闸门拒绝, 窗口无画面
    (属预期); 延时拍照窗口同为 04:30~18:00, 一致


2026-08-15 (傍晚)  延时摄影改白天限定 (04:30~18:00)

  [需求] 用户: 摄像头无夜视, 每天 18:00~次日 4:30 不需要拍

  [实现] 定时器保持每 10 分钟触发不变, 在 timelapse_snap.sh 头部
    加时间闸门 (date +%H%M, 430<=HM<1800 才拍, 否则静默退出) —
    比 systemd calendar 表达式直观, 改时间段只需改一行

  [验证] 17:03 手动触发拍照成功 (窗口内); 语法检查通过


2026-08-15 (午后4)  摄像头画面: 旋转/放大按钮 + 延时照片自动转正

  [需求] 用户确认软件方案: 查看页加旋转和放大按钮; 照片软件转正
    (摄像头在花园侧装, 画面横 90°)

  [改动] (192.168.3.41)
    * camera_web/index.html: 右下浮动工具栏 －/＋/⟳ 三个 64px
      触控按钮 (Pi 触屏可点); 旋转 90° 步进循环, 放大 1.5x 步进
      (1~6x); 状态存 localStorage 跨重载保持; 横竖态切换时
      max-width/max-height 互换保证适配
    * timelapse_snap.sh: 存盘后经 jpegtran -rotate 270 -perfect
      无损转正 (libjpeg-turbo-progs 已装); || true 兜底, 旋转失败
      不丢照片
    * 用户上一需求: latest.jpg 软链已去除 (本轮回滚确认)

  [验证] 手动触发拍照, rose 照片转正正确 (pot 朝下, 新芽朝上);
    grim 实机截图确认查看页三按钮渲染在画面右下

  [注意] 若日后物理摆正摄像头: 去掉 timelapse_snap.sh 的
    jpegtran 行 + 查看页把旋转点回 0° (localStorage 有记忆)
...
2026-08-14  ESP32 湿度公式修正 + ADC 悬空负值归零 (esp32/main.py v4.1)
  ...
  [反馈] 用户报实际湿度仅 75%, 面板却显示 ~93%; 且 A0/A1 未接
    任何东西, 应显示 0V 却显示 -0.246V

  [湿度根因] read_sht30() 湿度公式误用 -6 + 125*raw/65535, 这是
    温度式子的斜率和截距(且截距还是错的); SHT30 数据手册湿度应为
    100 * raw / 65535。改后实测 92.8% -> 75.5%, 符合实际

  [ADC 根因] ADS1115 配成单端(AINx vs GND), 物理上不可为负;
    悬空引脚高阻噪声产生稳定 -0.246V 负偏置。read_ads1115() 里
    采样值 <0 归零, 实测 -0.246 -> 0.0 (真实正信号不受影响)

  [刷机] ESP32 接回 PC COM5, mpremote cp + reset (WiFi 联网后
    MQTT 实时验证)。注意: 设备在 Pi 侧无串口时是纯 WiFi 独立运行,
    刷固件必须先 USB 直连 PC 或 Pi
...
2026-08-10  修复天气屏风向罗盘箭头方向反了 (weather.html)
  ...
  [问题] 用户从 VNC 截图发现风卡片罗盘箭头与「方向」文字矛盾:
    显示「东风 80°」但箭头指向东。Open-Meteo wind_direction_10m
    是风的来向 (80°东风=风从东来), 而 compassSVG 把箭头尖端直接
    画在该角度上 = 指向来向; 苹果天气惯例箭头应指风的去向

  [修复] pi_display/weather.html compassSVG(): 箭头角度 +180°
    (dir=(deg+180)%360), 一行改动

  [部署验证] scp 到 Pi 重启 kiosk, 用 #p2 详情页 grim 截图确认:
    「东风 79°」箭头指向西, 正确; 验证后 wx_launch.sh 恢复 P1

  [提交] 4a4adc6, 已推送 GitHub
...
2026-08-09 (晚11)  天气屏 v7.3 - P3 温度单位 °C + 状态卡分行
  ...
  [反馈] 用户截图指出: 室外温度卡单位只显示 "°" 不对; 传感器状态卡
    "更新于/信号/已运行" 挤一行随意折行 ("已运/行" 被截断)

  [改动] weather.html v7.3:
    * 温度卡 <small>°</small> → <small>°C</small>; 24h 极值行同步
      "29.19°C · 最高 30.83°C"
    * 状态卡 oStD 改 innerHTML + <br> 三行: 更新于 / 信号 / 已运行

  [验证] 本地预览 (http.server + 1024x600, 直连 Pi dashboard 真实
    数据): oT="30.4°C", oStD 三行, 卡片无溢出 (scrollHeight≤
    clientHeight), 首卡 x=10 末卡右缘 1014 满宽; scp 部署 +
    wx_launch.sh 重启, grim 确认 P1 正常


2026-08-09 (晚10b)  天气屏 v7.2 - P3 实时卡轮询 30s→2s

  [现象] 用户手电照 BH1750, 屏幕约 20 秒才反映, 质疑"不是 2s 吗"

  [定位] 数据链路确实是 2s (collector 日志/DB 行间隔实测 2s,
    22:28:55→22:28:57 lux 37.5→38.3); 延迟在显示层 — weather.html
    P3 实时卡 setInterval(pullOutdoor,30000)。30s 轮询 + 最坏 2s
    管线 ≈ 用户感知的 20 秒。**教训: "2s 更新"需求要穿透到显示层,
    采样/入库/展示三段间隔需分别确认**

  [改动] weather.html v7.2: pullOutdoor 30000→2000 (曲线卡仍
    10 分钟); scp 部署 + wx_launch.sh 重启 kiosk 立即生效;
    grim 截图确认 P1 正常渲染

  [负载] /api/current 为两条小 SQLite 查询, 本机 2s 一次无压力
    (weather 拉取在后台线程缓存, 不会阻塞, 见 v5.6)
...
2026-08-09 (晚7)  天气显示屏 v7.1 - P3 微调 + ESP32 掉线根因定位
  ...
  [改动] P3 「7天温度」卡改为「24小时湿度」曲线 (用户要求; 7天数据
    尚少, 后续可在 dashboard /weather 页看); 曲线极值标注加 clamp
    防左缘裁切 ("29.55" 曾显示为 "9.55")

  [排查] 用户报告 P3 状态卡在离线/在线间反复。DB 分析 uptime/
    counter 序列: 30 分钟内 2 次硬重启 (uptime 归零) + 多次 40~330s
    数据缺口, 2s 节奏丢包率 84%。RSSI -50~-60 正常。结论: 非软件
    问题, 疑似 USB 供电不稳 (经 1a86 集线器) — 同一晚 CH340 串口
    芯片死锁、WiFi 掉线 12 分钟、反复 brownout 重启, 均指向电源/
    线材。建议: ESP32 直连 Pi USB 口 (不经过集线器) + 换好线。
    待串口恢复后可刷 v3.2 固件 (计划: WDT 看门狗 + 发布失败时
    关闭旧 socket + 重连退避 5s->2s)


2026-08-09 (晚6)  室外传感器管线: 2s 上报 + 分层存储 + 天气屏 P3

  [需求] ESP32 移室外 (USB 供电), 数据上报 Pi5; Pi 上可查一周全部
  数据, 天气屏直接看室外实时值。用户定: 上报 2s, 保留 2 年

  [架构] 分层存储: 原始数据 (2s/条) 保留 8 天, sensor_minute
  1 分钟聚合表保留 2 年 (30M 行原始数据 ≈ 十几 GB, 聚合后 2 年
  仅 ~1M 行 / 几十 MB)

  [改动]
    * esp32/main.py v3.1: 上报 10s->2s; 主循环加 WiFi 断线重连
      (v3.0 掉线后永不重连)。实战检验: 部署当晚 WiFi 掉线 12 分钟,
      重连代码自动恢复上报 (uptime 未归零证明没重启)
    * mqtt_collector.py v3.2: sensor_minute 聚合表 (5 分钟增量聚合,
      INSERT OR REPLACE 幂等); 原始 8 天/聚合 2 年滚动删除;
      (device,timestamp) 索引; WAL; 启动 VACUUM
    * dashboard.py v5.2: /api/outdoor?field=&hours= (<=24h 原始表
      5分钟桶 / <=48h 1小时桶 / 更长走聚合表); after_request 加
      CORS 头 (天气屏 file:// 跨域); 字段白名单防注入
    * weather.html v7.0: 新增 P3 室外页 (4x3): 温度/湿度/气压/光照
      大卡 + 24h温度 + 7天温度 + 24h光照三条曲线 + 状态卡 (在线/
      RSSI/更新于) + ADC 卡; 30s 轮询 /api/current, 10 分钟拉曲线;
      离线数值变灰; pager 200vw->300vw, 圆点 3 个, 支持 #p3

  [事件] v3.1 部署后约 50 秒设备停报 (WiFi 掉线 12 分钟, 由新增的
    重连代码自愈, uptime 未归零), 同期 CH340 串口芯片死锁 (ioctl
    超时, unbind/bind 无法恢复)。两者独立: 数据走 WiFi 不受影响;
    串口死锁期间 mpremote 不可用, 需物理拔插 USB 恢复。观察项:
    疑似 USB 供电/集线器问题, 若再发生需排查

  [验证] DB 2s 节奏确认; 聚合 SQL 手动验证; API 两档 curl 验证;
    P3 实机 grim (#p3 直达)
